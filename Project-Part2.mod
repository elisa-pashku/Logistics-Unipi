...
minimize Assignment_Cost:sum {i in Tasks, j in Workers}10*assignedTasks[i,j]+150*Exceed;
...
subject to Constr_if: sum {i in Tasks}Duration[i]*assignedTasks[i,1]<=9+100*Exceed